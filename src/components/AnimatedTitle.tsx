...
export default function AnimatedTitle() {
  ...
  const scraping = useAnimationControls();
  const discordBot = useAnimationControls();
  const automation = useAnimationControls();
  ...
  useEffect(() => {
    ...
    const animationActions = [
      ...
      { controller: web, value: 'hidden' },
      ////////////////////////
      { controller: scraping, value: 'visible' },
      { controller: web, value: 1000 },
      { controller: scraping, value: 'hidden' },
      { controller: web, value: 'hidden' },
      { controller: discordBot, value: 'visible' },
      { controller: web, value: 1000 },
      { controller: discordBot, value: 'hidden' },
      { controller: web, value: 'hidden' },
      { controller: web, value: 700 },
      { controller: web, value: 'visible' },
      { controller: automation, value: 'visible' },
      { controller: web, value: 1000 },
      { controller: automation, value: 'hidden' },
      { controller: web, value: 'hidden' },
    ];
  ...
  }, [frontEnd, backEnd, web , scraping, discordBot, automation]);
  ...
  return (
    <div className="flex text-blue-100">
      ...
      <motion.div variants={list} initial="hidden" animate={scraping}>
        <WriteWord word="Scraping" />
      </motion.div>
      <motion.div variants={list} initial="hidden" animate={discordBot}>
        <WriteWord word="Discrod Bot" />
      </motion.div>
      <motion.div variants={list} initial="hidden" animate={automation}>
        <WriteWord word="Automation" />
      </motion.div>
    </div>
  );
}
